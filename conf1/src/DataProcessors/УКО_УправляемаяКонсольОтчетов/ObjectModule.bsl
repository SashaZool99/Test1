...
Функция УКО_Запрос_ВыполнитьЗапрос(Данные, Режим, АнализируетсяПланЗапроса)
	...
	ДополнитьМенеджерВТ(МенеджерВременныхТаблиц);
...
Процедура ДополнитьМенеджерВТ(МенеджерВТ)
	...
	Для каждого СтрокаДанныеМенджераВТ Из ДанныеДляМенеджераВТ Цикл
		
		Если НЕ СтрокаДанныеМенджераВТ.Использовать Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяВТ = СтрокаДанныеМенджераВТ.ИмяВТ;
		РезультатВТ = ПолучитьИзВременногоХранилища(СтрокаДанныеМенджераВТ.АдресВТ);
		
		Запрос.УстановитьПараметр(ИмяВТ, РезультатВТ);
		
		МассивТекстовЗапросов.Добавить(СтрШаблон(ШаблонТекстаЗапроса, ИмяВТ));
