...
Процедура ОбновитьЭлементыФормы ()
	...
	ИначеЕсли ЭтоСтрокаЗапросИлиПодЗапрос Тогда
		...
		// ++ UFG
		Элементы.ПараметрыЗапросаЗагрузитьПараметрыИзЗначениеСтрокиВнутр.Доступность = ДоступныДействияСПараметрами;
		// -- UFG
...
&НаКлиенте
Процедура ЗагрузитьПараметрыИзЗначениеСтрокиВнутр(Команда)
	НачатьЗагрузкуПараметровИзСтрокиВнутрСтрокой();
КонецПроцедуры

&НаКлиенте
Процедура НачатьЗагрузкуПараметровИзСтрокиВнутрСтрокой()
	
	Оповещение = Новый ОписаниеОповещения("ПослеВводаСтрокиИзСтрокиВнутрСтрокой", ЭтотОбъект);
	ПоказатьВводСтроки(Оповещение, "",,, Истина);

КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаСтрокиИзСтрокиВнутрСтрокой(Результат, ДополнительныеПараметры) Экспорт
	...
	Строка = Результат;
	СтруктураПараметров = ПодготовитьПараметрыИзЗначениеСтрокиВнутрНаСервере(Строка);
	
	Для каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		ЗначениеПараметра = Неопределено;
		Если СтруктураПараметров.Свойство(СтрокаПараметра.Имя, ЗначениеПараметра) <> Неопределено
				И ЗначениеПараметра <> Неопределено Тогда
				
			УКО_ДанныеКлиент_ВставитьЗначениеПараметраЗапроса(ЭтаФорма, ТекущаяСтрока, СтрокаПараметра.Имя, ЗначениеПараметра);
			ОбновитьСтрокуПараметра(СтрокаПараметра, СтрокаПараметра.Имя, СтрокаПараметра.ТипЗначения, ЗначениеПараметра); 
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПодготовитьПараметрыИзЗначениеСтрокиВнутрНаСервере(Строка)
	
	СтруктураПараметров = ЗначениеИзСтрокиВнутр(Строка);
	
	МассивКлючейТЗ = Новый Массив();
	Для каждого СтрокаПараметра Из СтруктураПараметров Цикл
			
			Тип = ТипЗнч(СтрокаПараметра.Значение);
			
			Если Тип = Тип("Массив") Тогда
				СЗЗначений = Новый СписокЗначений;
				СЗЗначений.ЗагрузитьЗначения(СтрокаПараметра.Значение);
				СтруктураПараметров.Вставить(СтрокаПараметра.Ключ, СЗЗначений);
			КонецЕсли;
			
			Если Тип = Тип("ТаблицаЗначений") Тогда
				МассивКлючейТЗ.Добавить(СтрокаПараметра.Ключ);
			КонецЕсли;
			
	КонецЦикла;
	
	Для каждого КлючКУдалению Из МассивКлючейТЗ Цикл
		СтруктураПараметров.Удалить(КлючКУдалению);
	КонецЦикла;
...
&НаКлиенте
Процедура ОчиститьМенеджерВТНаКлиенте()

	Объект.ДанныеДляМенеджераВТ.Очистить();

КонецПроцедуры 

&НаКлиенте
Процедура ДобавитьМенеджерВТ(Команда)
	
	НачатьДобавлениеМенеджераВТИзСтрокиВнутрСтрокой();
	
КонецПроцедуры

&НаКлиенте
Процедура НачатьДобавлениеМенеджераВТИзСтрокиВнутрСтрокой()
	
	Оповещение = Новый ОписаниеОповещения("ПослеВводаСтрокиДляДобавленияМенеджераВТИзСтрокиВнутрСтрокой", ЭтотОбъект);
	ПоказатьВводСтроки(Оповещение, "",,, Истина);

КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаСтрокиДляДобавленияМенеджераВТИзСтрокиВнутрСтрокой(Результат, ДополнительныеПараметры) Экспорт
	...
	ОчиститьМенеджерВТНаКлиенте();
	
	Строка = Результат;
	ЗаполнитьМенеджерВТНаСервере(Строка);
	
КонецПроцедуры 

&НаСервере
Процедура ЗаполнитьМенеджерВТНаСервере(СтрокаВнутр)

	СтруктураМенеджераВТ = ЗначениеИзСтрокиВнутр(СтрокаВнутр);
	
	Для каждого ЭлементСтруктурыМенеджераВТ Из СтруктураМенеджераВТ Цикл
	
		ИмяВТ = ЭлементСтруктурыМенеджераВТ.ИмяВТ;
		РезультатВТ = ЭлементСтруктурыМенеджераВТ.РезультатВТ;
		
		АдресВТ = ПоместитьВоВременноеХранилище(РезультатВТ, Новый УникальныйИдентификатор());
		
		НоваяСтрока = Объект.ДанныеДляМенеджераВТ.Добавить();
		НоваяСтрока.ИмяВТ = ИмяВТ;
		НоваяСтрока.АдресВТ = АдресВТ;
...
&НаКлиенте
Процедура ДанныеДляМенеджераВТПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	_ТекущаяСтрока = Элемент.ТекущиеДанные;
	
	Если НоваяСтрока Тогда 
		
		_ТекущаяСтрока.АдресВТ = АдресНовойТЗ();
		_ТекущаяСтрока.ИмяВТ = "ВременнаяТаблица";
...
&НаСервере
Функция АдресНовойТЗ()

	_ТаблицаЗначений = Новый ТаблицаЗначений;
	Возврат ПоместитьВоВременноеХранилище(_ТаблицаЗначений, Новый УникальныйИдентификатор());

КонецФункции

&НаСервере
Процедура ПослеЗавершенияРедактированияТаблицыМенеджераВТ(Таблица, ДополнительныеПараметры) Экспорт
	
	Если Таблица = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	_ТекущаяСтрока = Элементы.ДанныеДляМенеджераВТ.ТекущиеДанные;
	_ТекущаяСтрока.АдресВТ = ПоместитьВоВременноеХранилище(Таблица, _ТекущаяСтрока.АдресВТ);

КонецПроцедуры

&НаКлиенте
Процедура ОткрытьВТ(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗавершенияРедактированияТаблицыМенеджераВТ", ЭтотОбъект);
	
	ДополнительныеПараметрыОткрытия = Новый Структура;
	ДополнительныеПараметрыОткрытия.Вставить("ДополнениеЗаголовка", СтрШаблон("(%1): %2", НСтр("ru = 'Таблица менеджера'"), _ТекущаяСтрока.ИмяВТ));
	ДополнительныеПараметрыОткрытия.Вставить("ОписаниеОповещенияОЗакрытии", ОписаниеОповещения);
	
	УКО_ФормыКлиент_ОткрытьТаблицуЗначений(_ТекущаяСтрока.АдресВТ, ЭтаФорма);
